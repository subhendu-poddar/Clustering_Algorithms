...
float** finddistance(int r, int c, int k, float **b, float **cent, float **cent1, int iteration, int m1, int** elemlist) {
	...
	for(i=0;i<r;i++) {
		for(i1=0;i1<k;i1++) {
			dist[i1] = 0;
			for(j=0;j<c;j++) { //finding manhattan distances
				 mdist=b[i][j]-cent[i1][j];
				if(mdist>=0)
					dist[i1]=dist[i1]+mdist;
				else
					dist[i1]=dist[i1]-mdist;
		...
		// d1 gives the cluster to which the point  should go
		for(j=0;j<c;j++) {
			d2 = element[d1];
			list[d1][element[d1]][j] = b[i][j];
		}
		element[d1]++;
	...
	*/
	float **s;//updated centroids shall be stored
        s=(float **)malloc(k*sizeof(float *));
        for(i=0;i<k;i++)
                s[i]=(float *)malloc(c*sizeof(float));
	
	//centroid updation begins
	float temp;
	for(i=0;i<k;i++)
	{
		for(j=0;j<c;j++)
		{
			if(element[i]==0)
				s[i][j]=cent[i][j];
			else//median calculation starts
			{
			//first we have to arrange the elements in ascending order
				for(as1=0;as1<(element[i]-1);as1++)
				{
					for(as2=as1+1;as2<element[i];as2++)
					{
						if(list[i][as1][j]>list[i][as2][j])
						{
							temp=list[i][as2][j];
							list[i][as2][j]=list[i][as1][j];
							list[i][as1][j]=temp;
						}
					}
				}
				if(element[i]%2==0)
				{
					s[i][j]=(list[i][element[i]/2][j]+list[i][(element[i]/2)-1][j])/2;
				}
				else
					s[i][j]=list[i][element[i]/2][j];
			}
		}
	}
	...
	for(i=0;i<k;i++) {
		for(j=0;j<element[i];j++) {
			for(j1=0;j1<c;j1++) {
				if((list[i][j][j1]-cent[i][j1])>=0)
					sse=sse+(list[i][j][j1]-cent[i][j1]);
				else
					sse=sse-(list[i][j][j1]-cent[i][j1]);
			}
		}
	}
	//sum of errors (in k_median_clustering) are found
	cent[0][c]=sse;
	...
	if((cl_count != k*c) && iteration < 50)  {
		...
		return finddistance(r,c,k,b,cent,cent1,iteration,m1,elemlist);
	}
	else {
		for(i=0;i<k;i++) elemlist[m1][i]=element[i];
		cent[0][c] = sse;
		return cent;
	}
...
int main(int argc, char *argv[])
{
	...
	long m1,random,m2;
	float sse[50],***centlist, min =1000000;
	...
	for(m1=0;m1<f;m1++)
	{
		...
		cent = finddistance(p,w,k,b,cent,cent1,iteration,m1,elemlist);
	...
	for(i=0;i<k;i++) printf("%d  ",elemlist[m2][i]); printf("\n");
	printf("\nFinal centroids are\n\n");
	for(i=0;i<k;i++)
	{
		for(j=0;j<w;j++)
	...
	char sh[20];
	sprintf(sh, "_%ld_kmedians.data",k);
	strcat(argv[1],sh);
